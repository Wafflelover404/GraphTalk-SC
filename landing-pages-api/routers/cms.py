...
cms_security = HTTPBearer()

CMS_TOKEN = os.getenv("CMS_MASTER_TOKEN")

async def verify_cms_token(credentials: HTTPAuthorizationCredentials = Depends(cms_security)):
    if credentials.credentials != CMS_TOKEN:
...
async def get_blog_posts(_=Depends(verify_cms_token)):
    """Get all blog posts"""
...
async def create_blog_post(post: BlogPostCreate, _=Depends(verify_cms_token)):
...
async def update_blog_post(post_id: int, post: BlogPostUpdate, _=Depends(verify_cms_token)):
    """Update a blog post"""
    db = await get_db_connection()
    try:
        cursor = await db.execute(
...
async def delete_blog_post(post_id: int, _=Depends(verify_cms_token)):
    """Delete a blog post"""
    db = await get_db_connection()
    try:
        cursor = await db.execute(
...
async def create_help_article(article: HelpArticleCreate, _=Depends(verify_cms_token)):
...
async def update_help_article(article_id: int, article: HelpArticleUpdate, _=Depends(verify_cms_token)):
...
async def delete_help_article(article_id: int, _=Depends(verify_cms_token)):
...
async def upload_media(file: UploadFile = File(...), _=Depends(verify_cms_token)):
    """Upload media file"""
    try:
...
async def get_media_file(filename: str, _=Depends(verify_cms_token)):
    """Get media file"""
    try:
        upload_dir = os.getenv("UPLOAD_DIR", "./uploads")
...
async def delete_media_file(filename: str, _=Depends(verify_cms_token)):
    """Delete media file"""
    try:
        upload_dir = os.getenv("UPLOAD_DIR", "./uploads")
        file_path = os.path.join(upload_dir, filename)
...
async def get_content_stats(_=Depends(verify_cms_token)):
...
async def get_system_health(_=Depends(verify_cms_token)):
