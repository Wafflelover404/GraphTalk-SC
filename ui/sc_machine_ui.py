...
# API Client Page
    ...
    # API interaction tabs
    tab1, tab2 = st.tabs(["Query Knowledge Base", "Upload Knowledge Base"])
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            if data['status'] == "success":
                                ...
                                st.error(f"Query failed: {data['message']}")
                        else:
                            st.error(f"Server returned status code {response.status_code}")
                    except requests.exceptions.RequestException as e:
            ...
            if not uploaded_file:
                ...
                with st.spinner("Uploading and processing knowledge base..."):
                    try:
                        ...
                        response = requests.post(
                            f"{st.session_state.server_url.rstrip('/')}/upload/kb_zip",
                            files=files,
                            headers=headers
                        )
                        ...
                        if response.status_code == 200:
                            ...
                            if data['status'] == "success":
                                ...
                                st.error(f"Upload failed: {data['message']}")
                        else:
                            st.error(f"Server returned status code {response.status_code}")
                    except requests.exceptions.RequestException as e:
